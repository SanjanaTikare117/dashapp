...
async def get_csa_voltage_data():
    ...
    try:
        df_csa_voltage = pd.DataFrame({
            ...
            "CSA_A_Year": csa_voltage_df['CSA_A_Year'],
            "CSA_A_Month": csa_voltage_df['CSA_A_Month'],
            "CSA_A_day": csa_voltage_df['CSA_A_day'],
            "CSA_A_Hour": csa_voltage_df['CSA_A_Hour'],
            "CSA_A_datetime": csa_voltage_df['CSA_A_datetime']
        })
        json_output = df_csa_voltage.to_dict(orient="records")
        labels = get_labels(df_csa_voltage)
        return {"labels": labels, "data": json_output}
    except Exception as e:
        print(f"Error processing CSA voltage data: {e}")
        return {"detail": "Error processing CSA voltage data"}

# Endpoint to get data and labels for SM voltage dataset
@app.get("/sm_voltage")
async def get_sm_voltage_data():
    ...
    try:
        df_sm_voltage = pd.DataFrame({
            ...
            "SM_A_Year": sm_voltage_df['SM_A_Year'],
            "SM_A_Month": sm_voltage_df['SM_A_Month'],
            "SM_A_day": sm_voltage_df['SM_A_day'],
            "SM_A_Hour": sm_voltage_df['SM_A_Hour'],
            "SM_A_datetime": sm_voltage_df['SM_A_datetime']
        })
...
# Endpoint to get data and labels for DESE voltage dataset
...
async def get_dese_voltage_data():
    ...
    try:
        df_dese_voltage = pd.DataFrame({
            ...
            "DESE_A_Year": dese_voltage_df['DESE_A_Year'],
            "DESE_A_Month": dese_voltage_df['DESE_A_Month'],
            "DESE_A_day": dese_voltage_df['DESE_A_day'],
            "DESE_A_Hour": dese_voltage_df['DESE_A_Hour'],
            "DESE_A_datetime": dese_voltage_df['DESE_A_datetime']
        })
